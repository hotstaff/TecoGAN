...
for folder_i in range(folder_n):
        ...
        if "tOF" in keys:  # tOF
            ...
            if i > cutfr:  # temporal metrics
                target_OF = cv2.calcOpticalFlowFarneback(
                    pre_tar_grey, target_grey, None, 0.5, 3, 15, 3, 5, 1.2, 0)
                output_OF = cv2.calcOpticalFlowFarneback(
                    pre_out_grey, output_grey, None, 0.5, 3, 15, 3, 5, 1.2, 0)
                ...
                if False:  # for motion visualization
                    ...
                    mag, ang = cv2.cartToPolar(OF_diff[..., 0], OF_diff[...,
                                                                        1])
                ...
                OF_diff = np.sqrt(np.sum(OF_diff * OF_diff,
                                         axis=-1))  # l1 vector norm
    ...
    pd.DataFrame(pd_dict).to_csv(os.path.join(FLAGS.output, "metrics.csv"),
                                 mode=mode)
...
pd.DataFrame(avg_dict).to_csv(os.path.join(FLAGS.output, "metrics.csv"),
                              mode='a')
pd.DataFrame(folder_dict).to_csv(os.path.join(FLAGS.output, "metrics.csv"),
                                 mode='a')
pd.DataFrame(sum_dict).to_csv(os.path.join(FLAGS.output, "metrics.csv"),
                              mode='a')
print("Finished.")
