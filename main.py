...
flags.DEFINE_string(
    'vgg_ckpt', None,
    'path to checkpoint file for the vgg19')
...
flags.DEFINE_float(
    'warp_scaling', 1.0,
    'The scaling factor for the warp')
flags.DEFINE_boolean(
    'pingpang', False,
    'use bi-directional recurrent or not')
flags.DEFINE_float(
    'pp_scaling', 1.0,
...
flags.DEFINE_float(
    'decay_rate', 0.5,
    'The decay rate of each decay step')
flags.DEFINE_boolean(
    'stair', False,
...
flags.DEFINE_integer(
    'max_epoch', None,
    'The max epoch for the training')
flags.DEFINE_integer(
    'max_iter', 1000000,
...
flags.DEFINE_integer(
    'summary_freq', 100,
    'The frequency of writing summary')
flags.DEFINE_integer(
    'save_freq', 10000,
...
def printVariable(scope, key=tf.compat.v1.GraphKeys.MODEL_VARIABLES):
    ...
    variables_names = [[v.name, v.get_shape().as_list()]
                       for v in tf.compat.v1.get_collection(key, scope=scope)]
    total_sz = 0
