    ...
    with tf.device('/cpu:0'):
        ...
        with tf.compat.v1.variable_scope('load_frame'):
            for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
                inputDir = os.path.join(
                    ...
                    '%s_%04d' % (FLAGS.input_video_pre, dir_i))
                if os.path.exists(
                        inputDir
                ):  # the following names are hard coded: col_high_
                    if not os.path.exists(
                            os.path.join(inputDir,
                                         'col_high_%04d.png' % FLAGS.max_frm)):
                        print(
                            "Skip %s, since foler doesn't contain enough frames!"
                            % inputDir)
                        continue
            ...
            tensor_set_lists = tf.convert_to_tensor(image_set_lists,
                                                    dtype=tf.string)
            ...
            input_slices = input_slices[
                0]  # one slice contains pathes for (FLAGS.max_frm frames + 1) frames

            HR_data = []
            for frame_i in range(FLAGS.max_frm + 1):
                HR_data_i = tf.image.convert_image_dtype(tf.image.decode_png(
                    tf.read_file(input_slices[frame_i]), channels=3),
                                                         dtype=tf.float32)
                HR_data += [HR_data_i]
            ...
            HR_data = tf.reshape(
                HR_data, (HR_shape[0], HR_shape[1], HR_shape[2], HR_shape[3]))
        ...
        with tf.name_scope('sequence_data_preprocessing'):
            ...
            if FLAGS.random_crop and FLAGS.mode == 'train':
                ...
                raise Exception(
                    'Not implemented'
                )  # train_data can have different resolutions, crop is necessary
            ...
            if FLAGS.movingFirstFrame:
                ...
                moving_decision = tf.random.uniform([sequence_length],
                                                    ...
                                                    1,
                                                    dtype=tf.float32)
                fix_off_h = tf.clip_by_value(
                    offset_h, 0, HR_shape[-3] - tar_size - range_pos[1])
                fix_off_w = tf.clip_by_value(
                    offset_w, 0, HR_shape[-2] - tar_size - range_pos[0])
            ...
            for fi in range(FLAGS.RNN_N):
                HR_sequence = HR_data[
                    fi:fi + sequence_length]  # sequence_length, h, w, 3
                ...
                if FLAGS.random_crop and FLAGS.mode == 'train':
                    ...
                    if FLAGS.movingFirstFrame:
                        ...
                        HR_sequence_crop_1 = tf.image.crop_to_bounding_box(
                            HR_data_0, fix_off_h + lefttop_pos[fi][1],
                            fix_off_w + lefttop_pos[fi][0], tar_size, tar_size)

                        HR_sequence_crop = tf.where(moving_decision < 0.7,
                                                    HR_sequence_crop,
                                                    HR_sequence_crop_1)
                        name_sequence = tf.where(moving_decision < 0.7,
                                                 name_sequence, name_sequence1)

                HR_sequence_crop.set_shape(
                    [sequence_length, tar_size, tar_size, 3])
        ...
        if len(target_images) != FLAGS.RNN_N:
            raise ValueError(
                'Length of target image sequence is incorrect,expected {}, got {}.'
                .format(FLAGS.RNN_N, len(target_images)))
...
def loadHR(FLAGS, tar_size):
    ...
    with tf.device('/cpu:0'):
        ...
        for dir_i in range(FLAGS.str_dir, FLAGS.end_dir + 1):
            inputDir = os.path.join(FLAGS.input_video_dir,
                                    '%s_%04d' % (FLAGS.input_video_pre, dir_i))
                ...
                for fi in range(FLAGS.RNN_N):
                    image_list_HR_r[fi] += [
                        ...
                        for frame_i in range(
                            fi, FLAGS.max_frm - FLAGS.RNN_N + fi + 1)
                    ]
        ...
        with tf.compat.v1.variable_scope('load_frame'):
            ...
            if FLAGS.movingFirstFrame and FLAGS.mode == 'train':  # our data augmentation, moving first frame to mimic camera motion
                ...
                pos_xy = tf.cumsum(offset_xy, axis=0,
                                   exclusive=True)  # relative positions
                min_pos = tf.reduce_min(pos_xy, axis=0)
                range_pos = tf.reduce_max(
                    pos_xy, axis=0) - min_pos  # [ shrink x, shrink y ]
                lefttop_pos = pos_xy - min_pos  # crop point
            ...
            for fi in range(FLAGS.RNN_N):
                ...
                if FLAGS.movingFirstFrame:
                    ...
                    HR_data_1 = tf.image.crop_to_bounding_box(
                        HR_data_0, lefttop_pos[fi][1], lefttop_pos[fi][0],
                        target_size[0] - range_pos[1],
                        target_size[1] - range_pos[0])
                    HR_data = tf.cond(moving_decision < 0.7,
                                      lambda: tf.identity(HR_data),
                                      lambda: tf.identity(HR_data_1))
                    output_names[fi] = tf.cond(
                        moving_decision < 0.7,
                        lambda: tf.identity(output_names[fi]),
                ...
                if (FLAGS.random_crop is True) and FLAGS.mode == 'train':
                    ...
                    for frame_t in range(FLAGS.RNN_N):
                        target_images[frame_t] = tf.image.crop_to_bounding_box(
                            target_images[frame_t], offset_h, offset_w,
                            tar_size, tar_size)
            ...
            with tf.compat.v1.variable_scope('random_flip'):
                ...
                if (FLAGS.flip is True) and (FLAGS.mode == 'train'):
                    ...
                    flip_decision = tf.random.uniform([],
                                                      ...
                                                      dtype=tf.float32)
                    for frame_t in range(FLAGS.RNN_N):
                        target_images[frame_t] = random_flip(
                            target_images[frame_t], flip_decision)
...
def frvsr_gpu_data_loader(FLAGS, useValData_ph):
    ...
    HR_images_vald = list(
        vald_batch_list[FLAGS.RNN_N::])  # test batch high-res images
    ...
    with tf.name_scope('load_frame_gpu'):
        with tf.device('/gpu:0'):
            ...
            S_targets_frames.set_shape(
                (FLAGS.batch_size, FLAGS.RNN_N, 4 * FLAGS.crop_size,
                 4 * FLAGS.crop_size, 3))
