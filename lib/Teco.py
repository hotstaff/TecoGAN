...
from lib.ops import (conv2, batchnorm, lrelu, denselayer, vgg_19,
                     upscale_four, preprocessLR, deprocess,
                     gif_summary)
        ...
        with tf.name_scope('vgg_norm'):
            for key in output:
                if (deep_list is None) or (key in deep_list):
                    ...
                    if norm_flag:
                        orig_len = tf.sqrt(
                            tf.reduce_sum(tf.square(orig_deep_feature),
                                          axis=[3],
                                          keepdims=True) + 1e-12)
...
def TecoGAN(r_inputs, r_targets, FLAGS, GAN_Flag=True):
    ...
    with tf.device('/gpu:0'), tf.compat.v1.variable_scope('fnet'):
        ...
        fnet_input = tf.reshape(
            fnet_input, (FLAGS.batch_size * (inputimages - 1), FLAGS.crop_size,
        ...
        gen_flow = tf.reshape(
            gen_flow,
            (FLAGS.batch_size,
             (inputimages - 1), FLAGS.crop_size * 4, FLAGS.crop_size * 4, 2))
        ...
        input_frames = tf.reshape(
            Frame_t, (FLAGS.batch_size * (inputimages - 1), FLAGS.crop_size,
                      FLAGS.crop_size, output_channel))

    # tf.contrib.image.dense_image_warp, only in tf1.8 or larger, no GPU support
    s_input_warp = tf.contrib.image.dense_image_warp(
        tf.reshape(Frame_t_pre,
                   (FLAGS.batch_size * (inputimages - 1), FLAGS.crop_size,
                    FLAGS.crop_size, output_channel)), gen_flow_lr
    ...
    with tf.compat.v1.variable_scope('generator'):
        ...
        for frame_i in range(inputimages - 1):
            ...
            gen_pre_output_reshape = tf.reshape(
                gen_pre_output_warp,
                (FLAGS.batch_size, FLAGS.crop_size, 4, FLAGS.crop_size, 4, 3))
            gen_pre_output_reshape = tf.transpose(gen_pre_output_reshape,
                                                  perm=[0, 1, 3, 2, 4, 5])
        ...
        gen_outputs.set_shape([
            FLAGS.batch_size, inputimages, FLAGS.crop_size * 4,
            FLAGS.crop_size * 4, 3
        ])
        ...
        gen_warppre.set_shape([
            FLAGS.batch_size, inputimages - 1, FLAGS.crop_size * 4,
            FLAGS.crop_size * 4, 3
        ])

    # these are necessary for losses
    s_gen_output = tf.reshape(gen_outputs,
                              (FLAGS.batch_size * inputimages,
                               FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3))
    s_targets = tf.reshape(r_targets,
                           (FLAGS.batch_size * inputimages,
                            FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3))

    update_list = [
    ]  # collect all the losses, show the original value before scaling
    update_list_name = []  # collect all the names of the losses

    if FLAGS.vgg_scaling > 0.0:
        with tf.name_scope('vgg_19'):
            vgg_layer_labels = [
                'vgg_19/conv2/conv2_2', 'vgg_19/conv3/conv3_4',
                'vgg_19/conv4/conv4_4', 'vgg_19/conv5/conv5_4'
            ]
            gen_vgg = VGG19_slim(s_gen_output,
                                 reuse=False,
                                 deep_list=vgg_layer_labels)
            target_vgg = VGG19_slim(s_targets,
                                    reuse=True,
                                    deep_list=vgg_layer_labels)
    ...
    if GAN_Flag:  # build the discriminator
        ...
        with tf.device('/gpu:0'), tf.name_scope('input_Tdiscriminator'):
            ...
            t_gen_output = tf.reshape(
                gen_outputs[:, :t_size, :, :, :],
                (FLAGS.batch_size * t_size, FLAGS.crop_size * 4,
                 FLAGS.crop_size * 4, 3))
            ...
            if not FLAGS.pingpang:  # backward motion has to be calculated
                fnet_input_back = tf.concat((r_inputs[:, 2:t_size:3, :, :, :],
                                             r_inputs[:, 1:t_size:3, :, :, :]),
                                            axis=-1)
                ...
                gen_flow_back = tf.reshape(
                    gen_flow_back,
                    (FLAGS.batch_size, t_size // 3, FLAGS.crop_size * 4,
                     FLAGS.crop_size * 4, 2))

                T_inputs_VPre_batch = tf.identity(
                ...
                T_inputs_VPre_batch = tf.identity(
                    gen_flow[:, 0:t_size:3, :, :, :])  # forward motion reused,
                T_inputs_V_batch = tf.zeros_like(
                    T_inputs_VPre_batch)  # no motion for middle frames,
                T_inputs_VNxt_batch = tf.identity(
                    gen_flow[:, -2:-1 -
                             t_size:-3, :, :, :])  # backward motion reused

            T_vel = tf.stack(
                [T_inputs_VPre_batch, T_inputs_V_batch, T_inputs_VNxt_batch],
                axis=2)
            # batch, t_size/3, 3, FLAGS.crop_size*4, FLAGS.crop_size*4, 2
            T_vel = tf.reshape(T_vel,
                               (FLAGS.batch_size * t_size, FLAGS.crop_size * 4,
                                FLAGS.crop_size * 4, 2))
        ...
        if (
            ...
            paddings = tf.constant([[0, 0], [offset_dt, offset_dt],
                                    [offset_dt, offset_dt], [0, 0]])
        ...
        with tf.name_scope('real_Tdiscriminator'):
            ...
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('tdiscriminator',
                ...
                real_warp = tf.reshape(
                    real_warp0,
                    (t_batch, 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4,
                     3))  #[tb,T=3,h,w,ch=3 for RGB]
                real_warp = tf.transpose(real_warp, perm=[
                    0, 2, 3, 4, 1
                ])  # [tb,h,w,RGB,3T], 3T are t-1, t and t+1
                real_warp = tf.reshape(
                    real_warp,
                    (t_batch, FLAGS.crop_size * 4, FLAGS.crop_size * 4,
                     9))  # [tb,h,w,RRRGGGBBB] RRR: Red_t-1, Red_t, Red_t+1
                if FLAGS.crop_dt < 1.0:
                    real_warp = tf.image.crop_to_bounding_box(
                        real_warp, offset_dt, offset_dt, crop_size_dt,
                        crop_size_dt)
                ...
                if FLAGS.Dt_mergeDs:  # a spatio-temporal D
                    ...
                    with tf.compat.v1.variable_scope(
                        ...
                        before_warp = tf.reshape(
                            t_targets,
                            (t_batch, 3, FLAGS.crop_size * 4,
                             FLAGS.crop_size * 4, 3))  #[tb,3=T,h,w,3=RGB]
                        before_warp = tf.transpose(before_warp,
                                                   perm=[0, 2, 3, 4,
                                                         1])  # [b,h,w,3RGB,3T]
                        before_warp = tf.reshape(
                            before_warp,
                            (t_batch, FLAGS.crop_size * 4, FLAGS.crop_size * 4,
                             3 * 3))  # [b,h,w,9=RRRGGGBBB]

                        t_input = tf.reshape(r_inputs[:, :t_size, :, :, :],
                                             (t_batch, 3, FLAGS.crop_size, FLAGS.crop_size, -1))
                        t_input = tf.transpose(
                            t_input, perm=[0, 2, 3, 4,
                                           1])  # [tb,h//4,w//4,3RGB,3T]
                        t_input = tf.reshape(
                            t_input,
                        ...
                        real_warp = tf.concat(
                            (before_warp, real_warp, input_hi),
                            axis=-1)  # [tb,h,w,9 + 9 + 9]
        ...
        with tf.name_scope('fake_Tdiscriminator'):
            ...
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope(
                ...
                fake_warp = tf.reshape(
                    fake_warp0,
                    (t_batch, 3, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 3))
                fake_warp = tf.transpose(fake_warp, perm=[0, 2, 3, 4, 1])
                fake_warp = tf.reshape(
                    fake_warp,
                    (t_batch, FLAGS.crop_size * 4, FLAGS.crop_size * 4, 9))
                if FLAGS.crop_dt < 1.0:
                    fake_warp = tf.image.crop_to_bounding_box(
                        fake_warp, offset_dt, offset_dt, crop_size_dt,
                        crop_size_dt)
                ...
                if FLAGS.Dt_mergeDs:  # a spatio-temporal D
                    ...
                    with tf.compat.v1.variable_scope(
                        ...
                        before_warp = tf.reshape(t_gen_output,
                                                 (t_batch, 3, FLAGS.crop_size *
                                                  4, FLAGS.crop_size * 4, 3))
                        before_warp = tf.transpose(before_warp,
                                                   perm=[0, 2, 3, 4, 1])
                        before_warp = tf.reshape(before_warp,
                                                 (t_batch, FLAGS.crop_size * 4,
                                                  FLAGS.crop_size * 4, 3 * 3))
                        # input_hi is reused
                        fake_warp = tf.concat(
                            (before_warp, fake_warp, input_hi),
                            axis=-1)  # [tb,h,w,9 + 9 + 9]
                    tdiscrim_fake_output, fake_layers = discriminator_F(
                        fake_warp, FLAGS=FLAGS)
                else:
            ...
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('layer_loss'):
                ...
                for layer_i in range(layer_n):
                    ...
                    layer_loss = tf.reduce_mean(
                        tf.reduce_sum(tf.abs(layer_diff), axis=[3]))
                    # an l1 loss
                    layer_loss_list += [layer_loss]

                    scaled_layer_loss = Fix_Range * layer_loss / layer_norm[
                        layer_i]

                    sum_layer_loss += scaled_layer_loss
                    if Fix_margin > 0.0:
                        d_layer_loss += tf.maximum(
                            0.0, Fix_margin - scaled_layer_loss)

                update_list += layer_loss_list
                update_list_name += [("D_layer_%d_loss" % _)
    ...
    with tf.compat.v1.variable_scope('generator_loss'):
        ...
        with tf.device('/gpu:0'), tf.compat.v1.variable_scope('content_loss'):
            ...
            content_loss = tf.reduce_mean(
                tf.reduce_sum(tf.square(diff1_mse), axis=[3]))
        ...
        with tf.compat.v1.variable_scope('warp_loss'):
            ...
            warp_loss = tf.reduce_mean(
                tf.reduce_sum(tf.square(diff2_mse), axis=[3]))
        ...
        if FLAGS.vgg_scaling > 0.0:
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('vgg_layer_loss'):
                ...
                for layer_i in range(vgg_layer_n):
                    curvgg_diff = tf.reduce_sum(
                        gen_vgg[vgg_layer_labels[layer_i]] *
                        target_vgg[vgg_layer_labels[layer_i]],
                        axis=[3])
                    # cosine similarity, -1~1, 1 best
                    curvgg_diff = 1.0 - tf.reduce_mean(
                        curvgg_diff)  # 0 ~ 2, 0 best
            ...
            update_list_name += [
                "vgg_loss_%d" % (_ + 2) for _ in range(len(vgg_loss_list) - 1)
            ]
        ...
        if FLAGS.pingpang:
            with tf.device('/gpu:0'), tf.compat.v1.variable_scope('bidirection_loss'):
                ...
                # an l1 loss
                pploss = tf.reduce_mean(
                    tf.abs(gen_out_first - gen_out_last_rev))
    ...
    if GAN_Flag:  # Build the discriminator loss
        with tf.device('/gpu:0'), tf.compat.v1.variable_scope('t_discriminator_loss'):
            ...
            t_balance = tf.reduce_mean(
                t_discrim_real_loss) + t_adversarial_loss
            ...
            update_list += [
                tf.reduce_mean(tdiscrim_real_output),
                tf.reduce_mean(tdiscrim_fake_output)
            ]
            update_list_name += [
                "t_discrim_real_output", "t_discrim_fake_output"
            ]
    ...
    with tf.compat.v1.variable_scope('generator_train'):
        ...
        if not GAN_Flag:
            gen_grads_and_vars = gen_optimizer.compute_gradients(
                gen_loss, gen_tvars)
            fnet_grads_and_vars = fnet_optimizer.compute_gradients(
                fnet_loss, fnet_tvars)
            ...
            with tf.control_dependencies(
                ...
                counter1 = tf.compat.v1.get_variable(dtype=tf.int32,
                                                     shape=(),
                ...
                def train_gen_withD():
                    ...
                    with tf.control_dependencies([
                        ...
                        gen_grads_and_vars1 = gen_optimizer.compute_gradients(
                            gen_loss, gen_tvars)
                        fnet_grads_and_vars1 = fnet_optimizer.compute_gradients(
                            fnet_loss, fnet_tvars)
                        gen_train1 = gen_optimizer.apply_gradients(
                            gen_grads_and_vars1)
                        fnet_train1 = gen_optimizer.apply_gradients(
                            fnet_grads_and_vars1)
                ...
                def train_gen_withoutD():
                    ...
                    with tf.control_dependencies([tf.compat.v1.assign_add(counter2, 1), update_tb]):
                        ...
                        fnet_grads_and_vars2 = fnet_optimizer.compute_gradients(
                            fnet_loss, fnet_tvars)
                        gen_train2 = gen_optimizer.apply_gradients(
                            gen_grads_and_vars2)
                        fnet_train2 = gen_optimizer.apply_gradients(
                            fnet_grads_and_vars2)
    ...
    with tf.name_scope('image_summaries'):
        ...
        gif_sum = [
            ...
            gif_summary('HR',
                        ...
                        fps=3),
            gif_summary('Generated',
                        deprocess(gen_outputs),
                        max_outputs=max_outputs,
                        fps=3),
            gif_summary('WarpPreGen',
                        deprocess(gen_warppre),
                        max_outputs=max_outputs,
                        fps=3),
        ]
    ...
    Network = collections.namedtuple(
        'Network', 'gen_output, train, learning_rate, update_list, '
