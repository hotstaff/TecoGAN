...
def upscale_four(inputs, scope='upscale_four'):
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        p_inputs = tf.concat((inputs, inputs[:, -1:, :, :]),
                             axis=1)  # pad bottom
        ...
        for hi in range(4):
            for wj in range(4):
                hi_res_array.append(hi_res_bin[0][0] * (1.0 - 0.25 * hi) *
                                    (1.0 - 0.25 * wj) + hi_res_bin[0][1] *
                                    (1.0 - 0.25 * hi) * (0.25 * wj) +
                                    hi_res_bin[1][0] * (0.25 * hi) *
                                    (1.0 - 0.25 * wj) + hi_res_bin[1][1] *
                                    (0.25 * hi) * (0.25 * wj))
...
def bicubic_four(inputs, scope='bicubic_four'):
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        p_inputs = tf.concat((p_inputs[:, :, :1, :], p_inputs),
                             axis=2)  # pad left
        p_inputs = tf.concat(
...
def tf_data_gaussDownby4(HRdata, sigma=1.5):
    ...
    gau_list = np.float32([[gau_k, gau_0, gau_0], [gau_0, gau_k, gau_0],
                           [gau_0, gau_0,
                            gau_k]])  # only works for RGB images!
    gau_wei = np.transpose(gau_list, [2, 3, 0, 1])

    with tf.device('/gpu:0'):
        fix_gkern = tf.constant(gau_wei,
                                dtype=tf.float32,
                                shape=[k_w, k_w, 3, 3],
                                name='gauss_blurWeights')
        # shape [batch_size, crop_h, crop_w, 3]
        cur_data = tf.nn.conv2d(HRdata,
                                fix_gkern,
                                strides=[1, 4, 4, 1],
                                padding="VALID",
                                name='gauss_downsample_4')

        return cur_data


### Helper functions for model loading ############################################################
def get_existing_from_ckpt(ckpt,
                           var_list=None,
                           rest_zero=False,
                           print_level=1):
    ...
    for var in var_list:
        ...
        if reader.has_tensor(tensor_name):
            ...
            if print_level >= 2:
                print("loading tensor: " + str(var.name) + ", shape " +
                      str(npvariable.shape))
            if var.shape != npvariable.shape:
                raise ValueError(
                    'Wrong shape in for {} in ckpt,expected {}, got {}.'.
                    format(var.name, str(var.shape), str(npvariable.shape)))
...
def encode_gif(images, fps):
    ...
    cmd = [
        ...
        '[0:v]split[x][z];[z]palettegen[y];[x][y]paletteuse', '-r',
        '%.02f' % fps, '-f', 'gif', '-'
    ]
    ...
    for i in range(num_outputs):
        ...
        try:
            ...
            try:
                ...
                tf.logging.warning(
                    "Gif summaries requires ffmpeg or PIL to be installed: %s",
                    e)
                image_summ.encoded_image_string = "".encode(
                    'utf-8') if is_bytes else ""
        if num_outputs == 1:
